...
function taserProjectile::onCollision(%this, %obj, %col, %fade, %pos, %normal)
{
	...
	{
		%time = getSimTime();
        if(%time >= %obj.client.lastTase + 5000)
        {
            %col.setVelocity(VectorScale(getRandom(0, 0.250) SPC getRandom(0, 0.250) SPC "1", 10));
            tumble(%col);
            %obj.client.lastTase = %time;
        }

		if(!City_isLegalAttack(%obj.client, %col.client))
		{
			%obj.client.doCrime($Pref::Server::City::Crime::Demerits::tasingBros, "Tasing Innocents");
		}
	}
}

package CRPG_TaserPackage
{
...
activatePackage(CRPG_TaserPackage);
