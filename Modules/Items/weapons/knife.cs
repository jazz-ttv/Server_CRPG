...
//projectile
AddDamageType("knife", $City::DataPath @ '<bitmap:ui/ci/knife> %1', $City::DataPath @ '%2 <bitmap:ui/ci/knife> %1',0.75,1);
...
datablock ItemData(knifeItem)
{
    ...
    shapeFile = $City::DataPath @ "shapes/tools/knife/knife.dts";
    mass = 1;
    density = 0.2;
    elasticity = 0.2;
    friction = 0.6;
...
datablock ShapeBaseImageData(knifeImage)
{
   raycastWeaponRange 				 = 4;
	...
	raycastDirectDamage 			 = 5;
	raycastDirectDamageType 		 = $DamageType::knife;
	raycastExplosionProjectile 		 = knifeProjectile;
	raycastExplosionSound 			 = knifeHitSound;
   // Basic Item properties
   shapeFile = $City::DataPath @ "shapes/tools/knife/knife.dts";
   emap = true;
...
function knifeImage::onHitObject(%this, %obj, %slot, %col, %pos, %normal)
{
	...
	// 			if(%col.knifeHits > 0)
	// 			{
	// 				%client.centerPrint("<just:left>\c3Door:\c6" SPC %col.knifeHits @
	// 						"<br><just:left>\c0Breaking in", 3);
	// 				%col.knifeHits--;
	// 			}
	// 			else if(%col.knifeHits == 0)
	// 			{
