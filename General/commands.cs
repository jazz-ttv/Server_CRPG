package CRPG_Commands
{
	...
	// function serverCmddonate(%client, %amt)
	// {
	// 	%client.cityLog("/donate" SPC %amt);
	...
	// 	if(%amt * $Pref::Server::City::Economics::Price + $City::Economics::Condition > $Pref::Server::City::Economics::Cap) {
	// 		%amt = mFloor(($Pref::Server::City::Economics::Cap-$City::Economics::Condition) / $Pref::Server::City::Economics::Price);
	// 	}
	...
	// 	if((City.get(%client.bl_id, "money") - %amt) < 0)
	// 	{
	// 		messageClient(%client, '', "\c6You don't have that much money to donate.");
	// 		return;
	// 	}

	// 	%amoutPer = %amt * $Pref::Server::City::Economics::Price;
	// 	City.subtract(%client.bl_id, "money", %amt);
	// 	messageClient(%client, '', "\c6You donated " @ $c_p @ "$" @ %amt SPC "\c6to the economy. [" @ %amoutPer @ "%]");
	// 	messageAll('',$c_p @ %client.name SPC "\c6has donated " @ $c_p @ "$" @ %amt SPC "\c6to the economy [" @ %amoutPer @ "%]");
