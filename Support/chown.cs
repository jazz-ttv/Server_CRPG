...
if ($BLPrefs::Init)
{
	registerPreferenceAddon("Tool_ChangeOwnership", "Change Ownership", "brick_edit");
	new ScriptObject(Preference)
	{
		addon = "Tool_ChangeOwnership";
		category = "General";
		title = "Admin Command";
		type = "bool";
		variable = "$Pref::Server::CO::AdminCommand";
		defaultValue = false;
	};
	new ScriptObject(Preference)
	{
		addon = "Tool_ChangeOwnership";
		category = "General";
		title = "Force Create BrickGroup";
		type = "bool";
		variable = "$Pref::Server::CO::ForceCreateBrickGroup";
		defaultValue = true;
	};
	new ScriptObject(Preference)
	{
		...
		type = "num";
		params = "1 1000";
		variable = "$Pref::Server::CO::QueueSpeed";
		defaultValue = 100;
		hostOnly = true;
	};
	new ScriptObject(Preference)
	{
		...
		type = "num";
		params = "1 10000";
		variable = "$Pref::Server::CO::TransferSpeed";
		defaultValue = 1000;
		hostOnly = true;
	};
}
...
function Chown::tickGetBricks(%this, %i)
{
	...
	for (%limit = 0; %limit < %this.limit_bricks && %i < $Chown[%this, "Q"]; %limit++)
	{
		...
		// Cache the originating log trigger if there is one
		if(%brick.trigger)
			if(%brick.trigger.getDatablock() == CityRPGLotTriggerData.getID())
				$Chown[%this, "CacheTrigger"] = %brick.trigger;
		...
		for (%n = 0; %n < %count; %n++)
		{
			...
			// Put into queue if lot validation passes
			if($Chown[%this, "CacheTrigger"])
				if(%next.getCityBrickUnstable($Chown[%this, "CacheTrigger"]) == 0)
